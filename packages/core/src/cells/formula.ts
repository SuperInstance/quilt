...
/**
 * The signature of a compiled formula. Takes the cell values proxy
 * plus helpers, returns the computed value.
 *
 * Why positional args and not a single object? Because `new Function`
 * with named parameters gives us better stack traces and lets the
 ...
 */
function compile(expr: string, knownIds: Iterable<string>): CompiledFormula {
  ...
  return new Function(
    'cells',
    'abs', 'min', 'max', 'clamp', 'caller',
    `with (cells) { return (${body}); }`,
  ) as unknown as CompiledFormula;
}

// Helper functions exposed to formula bodies. Defined as constants so
// the compiler can reference them in the function call.
const helperAbs = (n: number) => Math.abs(n);
const helperMin = (...n: number[]) => Math.min(...n);
const helperMax = (...n: number[]) => Math.max(...n);
const helperClamp = (n: number, lo: number, hi: number) => Math.min(Math.max(n, lo), hi);
 ...
 */
export function evaluateFormula(cell: Cell, ctx: CallerContext, allCells: Map<CellId, Cell>): CellValue {
  if (!cell.def.expr) {
  ...
  try {
    ...
    const result = fn(
      cellProxy as unknown as Record<CellId, unknown>,
      helperAbs,
      helperMin,
      helperMax,
      helperClamp,
