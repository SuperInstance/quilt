 ...
 */
export function emptyContext(): CallerContext {
  return {
    trace: [],
    timestamp: Date.now(),
  };
 ...
 */
export function contextKey(ctx: CallerContext): string {
  const parts: string[] = [];
  if (ctx.row !== undefined) parts.push(`r:${String(ctx.row)}`);
  if (ctx.column !== undefined) parts.push(`c:${String(ctx.column)}`);
  if (ctx.caller) parts.push(`f:${ctx.caller}`);
  if (ctx.identity?.id) parts.push(`i:${ctx.identity.id}`);
  if (ctx.identity?.tags?.length) parts.push(`t:${ctx.identity.tags.sort().join(',')}`);
 ...
 */
export function evalWhen(when: string, ctx: CallerContext): boolean {
  try {
